...
def _write_tfr_trunk(
    ...
    with tf.io.TFRecordWriter(file+'-{}.tfr'.format(index)) as writer:
        for f in js_files:
            rd = convert_to_tfr(obj_mapper(all_df, _default_fs_mapper(f), lang, f), meta)
...
def write_objs_as_tfr(
    ...
    for s in tqdm(js_files):
        if len(cache) >= num_samples_to_infer_shape:
            break
        obj = obj_mapper(all_df, _default_fs_mapper(s), lang, s)
        if not obj:
            continue
        cache.append(obj)
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as e:
        for _ in tqdm(
            e.map(
                partial(
                    _write_tfr_trunk, all_df=all_df, lang=lang, obj_mapper=obj_mapper, meta=meta,
...
def make_batch(ds_name, ds, batch_size, map_parallelism=8,
