...
from keras.layers import Dense, Input, LSTM, Dropout, Bidirectional, Multiply, Dot, Lambda, Subtract
...
class SiameseBiLSTM:
    ...
    def train_model(self, sentences_pair, is_similar, eambedding_meta_data, language, model_save_directory='./'):
        """
        ...
        def custom_loss(y_truth, sim):
            ...
            loss = tf.reduce_mean(tf.where(tf.equal(y_truth, 1), 1 - sim, sim))
            return loss
            
        class PredictionCallback(tf.keras.callbacks.Callback):    
            def on_epoch_end(self, epoch, logs={}):
                y_pred = self.model.predict([train_data_x1, train_data_x2, leaks_train])
                print('prediction: {} epoch: {}'.format(y_pred[:50], epoch))
        ...
        print('custom_loss self-define lr=0.0005')
        nadam = keras.optimizers.Nadam(lr=0.0005, beta_1=0.9, beta_2=0.999)
        ...
        print('model saved at' + bst_model_path)
